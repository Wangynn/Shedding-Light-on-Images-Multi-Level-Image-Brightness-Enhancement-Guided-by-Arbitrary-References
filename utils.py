...
def match_feature(feature1, feature2):
    new_feature = tf.concat([feature1, feature2],1)
    return new_feature
...
def compute_ssim(img1, img2):
    img1_1 = img1[:,:,:,0:1]
    img2_1 = img2[:,:,:,0:1]
    ssim_1 = tf_ssim(img1_1, img2_1)
    img1_2 = img1[:,:,:,1:2]
    img2_2 = img2[:,:,:,1:2]
    ssim_2 = tf_ssim(img1_2, img2_2)
    img1_3 = img1[:,:,:,2:3]
    img2_3 = img2[:,:,:,2:3]
    ssim_3 = tf_ssim(img1_3, img2_3)
    ssim= (ssim_1 + ssim_2 + ssim_3)/3.0
    return ssim

def convertjpg(jpgfile, out_dir, width=400, height=600):
    img = Image.open(jpgfile)
    new_img = img.resize((width,height),Image.BILINEAR)
    [_, name] = os.path.split(jpgfile) 
    suffix = name[name.find('.') + 1:]  
    name = name[:name.find('.')]
    new_img.save(os.path.join(out_dir, '%s.png' % (name)))

def convertraw(rawfile, out_dir, width=400, height=600):
    image = rawpy.imread(rawfile)
    img = image.postprocess(use_camera_wb=True, half_size=False, no_auto_bright=True, output_bps=8)
    new_img = img.resize((width,height),Image.BILINEAR)
    [_, name] = os.path.split(rawfile) 
    suffix = name[name.find('.') + 1:]  
    name = name[:name.find('.')]
    imageio.imsave(out_dir+'%s.png' % (name), new_img)

def instance_normalization(instance):
    mean = tf.reduce_mean(instance)
    std = tf.sqrt(tf.reduce_mean(tf.square(mean - instance)))
    instance_norm = (instance - mean)/std
    return instance_norm

def computer_error(data1, data2):
    mean = np.mean(np.fabs(data1-data2))
    var = np.mean(np.square(mean-np.fabs(data1-data2)))
    # mean, var = tf.nn.moments(tf.abs(data1-data2),axes=1)
