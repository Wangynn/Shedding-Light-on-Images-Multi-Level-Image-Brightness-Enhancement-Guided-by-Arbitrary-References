...
train_low_data = []
train_high_data = []
train_low_data_names = glob('./Five_K/train/low/*.png')  #输出文件list：('…/low/1.png'，'…/low/2.png'，…)
train_low_data_names.sort(key=lambda x:(x[20],x[21],x[22],x[23]))
train_high_data_names = glob('./Five_K/train/high/*.jpg') 
train_high_data_names.sort(key=lambda x:(x[21],x[22],x[23],x[24]))
...
eval_low_data = []
eval_high_data = []
eval_low_data_names = glob('./Five_K/test/low/*.png')
eval_low_data_names.sort(key=lambda x:(x[19],x[20],x[21],x[22]))
eval_high_data_names = glob('./Five_K/test/high/*.jpg')
eval_high_data_names.sort(key=lambda x:(x[20],x[21],x[22],x[23]))
...
for epoch in range(start_epoch, epoch):  #循环次数为总epoch数
    for batch_id in range(start_step, numBatch):  #循环次数为一个epoch中的batch个数
        batch_input_low = np.zeros((batch_size, patch_size, patch_size, 3), dtype="float32")   #一个batch的低光输入
        ...
        for patch_id in range(batch_size):   #循环次数为一个batch中的patch个数
            ...
            batch_input_low[patch_id, :, :, :] = augmentation(train_low_data[image_id][x : x+patch_size, y : y+patch_size, :], rand_mode)   #数据增强
            batch_input_high[patch_id, :, :, :] = augmentation(train_high_data[image_id][x : x+patch_size, y : y+patch_size, :], rand_mode)
        ...
        _,  restoration_loss, feature_loss, content_loss, loss = sess.run([train_op, loss_restoration, loss_feature, loss_content, train_loss], feed_dict={input_low: batch_input_low, input_high: batch_input_high, lr: learning_rate})

        print("Epoch: [%2d] [%4d/%4d] time: %4.4f, restoration_loss: %.6f, feature_loss: %.6f, content_loss: %.6f, loss: %.6f, psnr: %.6f "\
                % (epoch + 1, batch_id + 1, numBatch, time.time() - start_time, restoration_loss, feature_loss, content_loss, loss, train_psnr))
        iter_num += 1
    ...
    if (epoch + 1) % eval_every_epoch == 0:
        ...
        for idx in range(len(eval_low_data_names)):
            ...
            save_images(os.path.join(sample_dir, '%s_%d-psnr:%.6f.png' % ( name, epoch + 1, eval_psnr)),  output, input_high_eval)
